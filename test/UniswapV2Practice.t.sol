...
contract UniswapV2PracticeTest is Test {
    ...
    address public taker = makeAddr("Taker"); //swap
    address public maker = makeAddr("Maker"); //add liquidity
    ...
    function test_taker_removeLiquidityETH() public {
        ...
        // Checking
        IUniswapV2Pair wethUsdcPair = IUniswapV2Pair(UNISWAP_V2_FACTORY.getPair(address(WETH9), address(testUSDC)));
        // Approve UNISWAP_V2_ROUTER contract to get the (wethUsdcPair) LP token.
        wethUsdcPair.approve(address(UNISWAP_V2_ROUTER), liquidity);
        // removeLiquidityETH
        UNISWAP_V2_ROUTER.removeLiquidityETH(address(testUSDC), liquidity, 0, 0, maker, block.timestamp + 10000);
        (uint112 reserve0, uint112 reserve1, ) = wethUsdcPair.getReserves();
        assertEq(reserve0, 1);
